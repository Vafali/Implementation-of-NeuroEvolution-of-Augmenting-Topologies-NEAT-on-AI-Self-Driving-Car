...
class Car:
    x = 0
    y = 0       #coordinates with respect to the global reference system, the position on the screen is relative to the best car position
    ...
    def getInputs(self, world, road):         #win is for drawing sensors if DBG=True
        sensors = []
        for k in range(8):
        ...
        if CAR_DBG:
            for k,s in enumerate(sensors):
                ...
                dx = s * sin(omega)
                dy = - s * cos(omega)
                #draw intersections of the sensors
    ...
    def move(self, road, t):
        ...
        self.y = self.y - self.vel * cos(radians(self.rot)) #I subtract because the origin is at the top left
...
def getSensorEquations(self, world):       #returns the equations of the straight lines (in variable y) of the car in order [vertical, diagonal increasing, horizontal, diagonal descending]
    eq = []
    for i in range(4):
...
def getSegmentEquation(p, q):          #equations in variable y between two points (taking into account the coordinate system with inverted y) in the general form ax + by + c = 0
...
def getDistance(world, car, sensors, sensorsEquations, p, q):     #given the segment (m,q) I calculate the distance and put it in the corresponding sensor
    ...
    for i,(a1,b1,c1) in enumerate(sensorsEquations):
        ...
        if a1!=a2 or b1!=b2:
            ...
            y = (a1*c2 - c1*a2)/d
            x = (c1*b2 - b1*c2)/d
            if (y-p.y)*(y-q.y) > 0 or (x-p.x)*(x-q.x) > 0:        #if the intersection doesn't fit between a and b, go to the next iteration
                continue
        ...
        omega = car.rot +45*i                               #angle of the straight line of the sensor (and its opposite)
        alpha = 90- degrees(atan2(car.y - y, x-car.x))     #angle to vertical (like car.rot)
